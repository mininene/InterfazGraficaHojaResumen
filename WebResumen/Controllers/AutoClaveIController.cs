   ...
   // [Authorize(Policy = "ADTodos")]
    public class AutoClaveIController : Controller
    {
        ...
        public async Task<IActionResult> Index(string nCiclo, string nPrograma, string fecha, int? page)
        {
            ...
            var query = _context.CiclosAutoclaves.Where(x => x.IdAutoclave == "NA0672EGI").AsNoTracking().AsQueryable();
        ...
        public async Task<IActionResult> CycleList(string ciclo, int? page)
        {
            ...
            {
                Cyclelist.Add(new DownloadViewModel { Dir = file.FullName, Ciclo = file.FullName.Split('\\')[5], Numero = Convert.ToInt32(file.FullName.Split('\\')[5].Split('I')[1].Split('.')[0]) });
            }
        ...
        public async Task<IActionResult> Download(string dir, string ciclo)
        {
            ...
            {
                var path = $"https://essahojaresumen.global.baxter.com/LOGFiles/AutoClaveI/{ciclo}";
