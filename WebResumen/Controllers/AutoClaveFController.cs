...
namespace WebResumen.Controllers
{
    ...
    public class AutoClaveFController : Controller
    {
        ...
        public AutoClaveFController(AppDbContext context, IPrinterOchoVeinte printerOchoVeinte, IPrinterDosTresCuatro printerDosTresCuatro, ILogRecord log,
        ...
        public async Task<IActionResult> Index(string nCiclo, string nPrograma, string fecha, int? page)
        {
            ...
            var query = _context.CiclosAutoclaves.Where(x => x.IdAutoclave == "NF1030F").AsNoTracking().AsQueryable();
        ...
        public async Task<IActionResult> Preview(int? id)
        {
           ...
           // string path = @"\\essaappserver01\HojaResumen\API\AutoClaveF\" + ciclo;
            var query = _context.MaestroAutoclave.Where(t => t.Matricula == "NF1030F").FirstOrDefault();
        ...
        public async Task<IActionResult> CycleList(string ciclo, int? page)
        {
            ...
            {
                var t  = file.FullName.Split('\\')[5].Split('F');
                Cyclelist.Add(new DownloadViewModel { Dir = file.FullName, Ciclo = file.FullName.Split('\\')[5], Numero = Convert.ToInt32(file.FullName.Split('\\')[5].Split('F')[2].Split('.')[0]) });
            }
        ...
        public async Task<IActionResult> Download(string dir, string ciclo)
        {
            ...
            {
                var path = $"https://essahojaresumen.global.baxter.com/LOGFiles/AutoClaveF/{ciclo}";
