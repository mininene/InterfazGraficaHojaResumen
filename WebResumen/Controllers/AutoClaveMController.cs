...
namespace WebResumen.Controllers
{
    ...
    public class AutoClaveMController : Controller
    {
        ...
        public async Task<IActionResult> Index(string nCiclo, string nPrograma, string fecha, int? page)
        {
            ...
            var query = _context.CiclosAutoclaves.Where(x => x.IdAutoclave == "NA0611EFM").AsNoTracking().AsQueryable();
        ...
        public async Task<JsonResult> ListAutoclaveM()
        {
            ...
            var query = from x in _sabiUno.Where(x => x.IdAutoclave == "NA0611EFM").OrderByDescending(X => X.Id).Take(50) select x;
        ...
        public async Task<JsonResult> ListaAutoclaveM()
        {
            //var result=  await _context.CiclosAutoclaves.OrderByDescending(x => x.Id).ToListAsync();
            //return View(await _context.CiclosAutoclaves.OrderByDescending(x=>x.Id).ToListAsync());
            List<CiclosAutoclaves> _sabiUno = await _context.CiclosAutoclaves.ToListAsync();
            var query = from x in _sabiUno.Where(x => x.IdAutoclave == "NA0611EFM").OrderByDescending(X => X.Id).Take(1) select x;
        ...
        public async Task<IActionResult> Print(int? id)
        {
           ...
           // if (ciclosAutoclaves.Programa.Trim().Equals("8") || ciclosAutoclaves.Programa.Trim().Equals("20"))
            ...
            _log.Write(_httpContextAccessor.HttpContext.Session.GetString("SessionFullName"), DateTime.Now, EventoM + " " + _httpContextAccessor.HttpContext.Session.GetString("AutoclaveNumeroM"), _httpContextAccessor.HttpContext.Session.GetString("SessionComentarioM"));
        ...
        public async Task<IActionResult> Preview(int? id)
        {
           ...
           // string path = @"\\essaappserver01\HojaResumen\API\AutoClaveM\" + ciclo;
            var query = _context.MaestroAutoclave.Where(t => t.Matricula == "NA0611EFM").FirstOrDefault();
        ...
        public async Task<IActionResult> CycleList(string ciclo, int? page)
        {
            ...
            {
                Cyclelist.Add(new DownloadViewModel { Dir = file.FullName, Ciclo = file.FullName.Split('\\')[5], Numero = Convert.ToInt32(file.FullName.Split('\\')[5].Split('M')[1].Split('.')[0]) });
            }
        ...
        public async Task<IActionResult> Download(string dir, string ciclo)
        {
            ...
            {
                var path = $"https://essahojaresumen.global.baxter.com/LOGFiles/AutoClaveM/{ciclo}";
        ...
        public async Task<IActionResult> Login(int? id)
        {
           ...
           // ViewBag.datos = ciclosAutoclaves.Id;
            HttpContext.Session.SetString("SessionDatosM", ciclosAutoclaves.Id.ToString());
            HttpContext.Session.SetString("AutoclaveNumeroM", ("AutoClaveM" + " " + "N°Ciclo:" + ciclosAutoclaves.NumeroCiclo).ToString());
                        ...
                        if (user != null)
                        {
                            if (user.IsMemberOf(groupAdmins) || user.IsMemberOf(groupSupervisors) || user.IsMemberOf(groupUsers))
                            {
                                using (var searcher = new DirectorySearcher(new DirectoryEntry(path)))
                                {
                                    ...
                                    HttpContext.Session.SetString("SessionComentarioM", model.Comentario);
        ...
        // GET: AutoClaveM/Create
        public IActionResult Create()
        {
            return View();
        }

        // POST: AutoClaveM/Create
