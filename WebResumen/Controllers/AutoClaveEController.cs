...
namespace WebResumen.Controllers
{
    ...
    public class AutoClaveEController : Controller
    {
        ...
        public async Task<IActionResult> Index(string nCiclo, string nPrograma, string fecha, int? page)
        {
            ...
            var query = _context.CiclosAutoclaves.Where(x => x.IdAutoclave == "NF1029E").AsNoTracking().AsQueryable();
        ...
        public async Task<JsonResult> ListAutoclaveE()
        {
        ...
        public async Task<JsonResult> ListaAutoclaveE()
        {
            //var result=  await _context.CiclosAutoclaves.OrderByDescending(x => x.Id).ToListAsync();
            //return View(await _context.CiclosAutoclaves.OrderByDescending(x=>x.Id).ToListAsync());
            List<CiclosAutoclaves> _sabiUno = await _context.CiclosAutoclaves.ToListAsync();
            var query = from x in _sabiUno.Where(x => x.IdAutoclave == "NF1029E").OrderByDescending(X => X.Id).Take(1) select x;
        ...
        public async Task<IActionResult> Print(int? id)
        {
           ...
           // if (ciclosAutoclaves.Programa.Trim().Equals("8") || ciclosAutoclaves.Programa.Trim().Equals("20"))
            ...
            _log.Write(_httpContextAccessor.HttpContext.Session.GetString("SessionFullName"), DateTime.Now, EventoD + " " + _httpContextAccessor.HttpContext.Session.GetString("AutoclaveNumeroE"), _httpContextAccessor.HttpContext.Session.GetString("SessionComentarioE"));

            return RedirectToAction("Index", "AutoClaveE");
        ...
        public async Task<IActionResult> WritePrint()
        {
            ...
            _log.Write(_httpContextAccessor.HttpContext.Session.GetString("SessionFullName"), DateTime.Now, EventoE + " " + _httpContextAccessor.HttpContext.Session.GetString("AutoclaveNumeroE"), _httpContextAccessor.HttpContext.Session.GetString("SessionComentarioE"));
        ...
        public async Task<IActionResult> Preview(int? id)
        {
           ...
           // string path = @"\\essaappserver01\HojaResumen\API\AutoClaveE\" + ciclo;
            var query = _context.MaestroAutoclave.Where(t => t.Matricula == "NF1029E").FirstOrDefault();
        ...
        public async Task<IActionResult> CycleList(string ciclo, int? page)
        {
            ...
            {
                Cyclelist.Add(new DownloadViewModel { Dir = file.FullName, Ciclo = file.FullName.Split('\\')[5], Numero = Convert.ToInt32(file.FullName.Split('\\')[5].Split('E')[1].Split('.')[0]) });
            }
        ...
        public async Task<IActionResult> Download(string dir, string ciclo)
        {
            ...
            {
                var path = $"https://essahojaresumen.global.baxter.com/LOGFiles/AutoClaveE/{ciclo}";
        ...
        public async Task<IActionResult> Login(int? id)
        {
            ...
            HttpContext.Session.SetString("AutoclaveNumeroE", ("AutoClaveE" + " " + "N°Ciclo:" + ciclosAutoclaves.NumeroCiclo).ToString());
